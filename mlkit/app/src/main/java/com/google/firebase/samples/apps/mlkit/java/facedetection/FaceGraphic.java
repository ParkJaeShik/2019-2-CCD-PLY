...
public class FaceGraphic extends Graphic {
    ...
    public void draw(Canvas canvas) {
        ...
        if (facing == CameraSource.CAMERA_FACING_FRONT) {
            ...
            float tmp1 = face.getLeftEyeOpenProbability();
            float tmp2 = face.getRightEyeOpenProbability();

            canvas.drawText(
                    "right eye: " + String.format("%.9f", tmp1),
                    x - ID_X_OFFSET, y, idPaint);
            canvas.drawText(
                    "left eye: " + String.format("%.9f", tmp2),
                    x + ID_X_OFFSET * 6, y, idPaint);
            ///////////////////////////////////////////////////////////////////////////////////////////////
            ///////////////////////////////////////////////////////////////////////////////////////////////
            /*if (face.getLeftEyeOpenProbability()==1.00 && face.getRightEyeOpenProbability()==1.00) {
                ((LivePreviewActivity)LivePreviewActivity.mContext).startIntentbutton();
            }*/
            //양안이 확률 90퍼센트 이상일 경우 intent를 실행한다.

            //String answer = LivePreviewActivity.myanswer;

            if (tmp1>=0.98 && tmp2>=0.98) {
                //intent_btn.performClick(); 여러번 클릭이 발생한다.
                /*if(answer.charAt(0)=='a') {
                    ((LivePreviewActivity) LivePreviewActivity.mContext).correctIntent();
                }
                if(answer.charAt(0)=='b') {
                    ((LivePreviewActivity) LivePreviewActivity.mContext).wrongIntent();
                }*/
